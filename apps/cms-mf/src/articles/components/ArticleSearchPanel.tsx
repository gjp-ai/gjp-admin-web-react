...
import type { ArticleSearchFormData } from '../types/article.types';
import { ARTICLE_TAG_SETTING_KEY, LANGUAGE_OPTIONS, ARTICLE_LANG_SETTING_KEY } from '../constants';

interface ArticleSearchPanelProps {
  searchFormData: ArticleSearchFormData;
  loading?: boolean;
  onFormChange: (field: keyof ArticleSearchFormData, value: any) => void;
  onSearch: () => void;
  onClear: () => void;
}

const ArticleSearchPanel: React.FC<ArticleSearchPanelProps> = ({
  searchFormData,
  loading,
  onFormChange,
  onSearch,
  ...
  const availableTags = useMemo(() => {
    try {
      ...
      console.error('[ArticleSearchPanel] Error loading tags:', err);
      return [] as string[];
    }
  }, [i18n.language]);
  ...
  const availableLangOptions = useMemo(() => {
    try {
      ...
      console.error('[ArticleSearchPanel] Error loading lang options:', err);
        ...
        >
          <Box>
            ...
            <TextField
              ...
              placeholder={t('articles.filters.searchByTitle')}
              disabled={loading}
              ...
              >
                ...
                {availableLangOptions.map((option) => (
                  <MenuItem key={option.value} value={option.value}>
                    {t(`articles.languages.${option.value}`)}
                  </MenuItem>
                ))}
              </Select>
            </FormControl>
              ...
              <Select<string[]>
                ...
                renderValue={(selected) => {
                  if (selected.length === 0) {
                    return <Typography variant='body2' color='text.disabled'>{t('articles.filters.all')}</Typography>;
                  }
                  return (
          ...
          <Box>
            ...
            <FormControl fullWidth size='small'>
              ...
              >
                <MenuItem value=''>{t('articles.filters.all')}</MenuItem>
                <MenuItem value='true'>{t('articles.status.active')}</MenuItem>
                <MenuItem value='false'>{t('articles.status.inactive')}</MenuItem>
              </Select>
            </FormControl>
          </Box>
        </Box>
