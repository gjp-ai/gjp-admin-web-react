...
import type { ImageSearchFormData } from '../types/image.types';
import { LANGUAGE_OPTIONS } from '../constants';
import { CHANNEL_OPTIONS } from '../../../../shared-lib/src';

interface ImageSearchPanelProps {
  searchFormData: ImageSearchFormData;
  loading: boolean;
  onFormChange: (field: keyof ImageSearchFormData, value: any) => void;
  onSearch: () => void;
  onClear: () => void;
}
...
const ImageSearchPanel: React.FC<ImageSearchPanelProps> = ({
  ...
  // Get image tags from local storage filtered by current language
  const availableTags = useMemo(() => {
    try {
      ...
      const imageTagsSetting = appSettings.find(
        (setting) =>
          setting.name === 'image_tags' && setting.lang === currentLang,
      );
      if (!imageTagsSetting) return [];
      return imageTagsSetting.value
        .split(',')
        .map((tag) => tag.trim())
        .filter(Boolean);
    } catch (error) {
      console.error('[ImageSearchPanel] Error loading tags:', error);
      return [];
    }
  }, [i18n.language]);
        ...
        >
          ...
          <Box>
            ...
            <TextField
              ...
              placeholder={t('images.filters.searchByName')}
              disabled={loading}
              sx={{
          ...
          <Box>
            ...
            <FormControl fullWidth size="small">
              ...
              >
                <MenuItem value="">{t('images.filters.all')}</MenuItem>
                {LANGUAGE_OPTIONS.map((option) => (
                  <MenuItem key={option.value} value={option.value}>
                    {t(`images.languages.${option.value}`)}
                  </MenuItem>
                ))}
              </Select>
            </FormControl>
          ...
          <Box>
            ...
            <FormControl fullWidth size="small">
              ...
              >
                <MenuItem value="">{t('images.filters.all')}</MenuItem>
                <MenuItem value="true">{t('images.status.active')}</MenuItem>
                <MenuItem value="false">{t('images.status.inactive')}</MenuItem>
              </Select>
            </FormControl>
          </Box>
        </Box>
