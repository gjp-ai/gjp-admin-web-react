...
const VideoSearchPanel: React.FC<VideoSearchPanelProps> = ({ searchFormData, loading, onFormChange, onSearch, onClear }) => {
  ...
  const availableTags = useMemo(() => {
    try {
      ...
      console.error('[VideoSearchPanel] Error loading tags:', err);
      return [] as string[];
    }
  }, [i18n.language]);
    ...
    <Card elevation={0} sx={{
      ...
      <CardContent sx={{ position: 'relative', zIndex: 2, p: 3 }}>
        ...
        <Box sx={{
          ...
          <Box>
            <FormLabel sx={{ fontWeight: 500, color: 'text.primary', mb: 1, display: 'block' }}>{t('videos.form.name') || t('videos.filters.searchByName')}</FormLabel>
            <TextField fullWidth size="small" value={searchFormData.name || ''} onChange={(e) => onFormChange('name', e.target.value)} placeholder={t('videos.filters.searchByName')} disabled={loading} sx={{ '& .MuiOutlinedInput-root': { borderRadius: 2 } }} />
          </Box>

          <Box>
            <FormLabel sx={{ fontWeight: 500, color: 'text.primary', mb: 1, display: 'block' }}>{t('videos.form.lang')}</FormLabel>
            <FormControl fullWidth size="small">
              <Select value={searchFormData.lang || ''} onChange={(e) => onFormChange('lang', e.target.value)} disabled={loading} displayEmpty sx={{ borderRadius: 2 }}>
                <MenuItem value="">{t('videos.filters.all')}</MenuItem>
                {LANGUAGE_OPTIONS.map((option) => (
                  <MenuItem key={option.value} value={option.value}>{t(`videos.languages.${option.value}`)}</MenuItem>
                ))}
              </Select>
            </FormControl>
          </Box>
          ...
          <Box>
            <FormLabel sx={{ fontWeight: 500, color: 'text.primary', mb: 1, display: 'block' }}>{t('videos.form.channel') || 'Channel'}</FormLabel>
            <FormControl fullWidth size="small">
          ...
          <Box>
            <FormLabel sx={{ fontWeight: 500, color: 'text.primary', mb: 1, display: 'block' }}>{t('videos.form.tags')}</FormLabel>
            <FormControl fullWidth size="small">
              <Select<string[]>
                multiple
                ...
                renderValue={(selected) => {
                  if (selected.length === 0) return (<Typography variant="body2" color="text.disabled">{t('videos.filters.all')}</Typography>);
          ...
          <Box>
            <FormLabel sx={{ fontWeight: 500, color: 'text.primary', mb: 1, display: 'block' }}>{t('videos.form.status') || 'Status'}</FormLabel>
            <FormControl fullWidth size="small">
              <Select value={searchFormData.isActive || ''} onChange={(e) => onFormChange('isActive', e.target.value)} disabled={loading} displayEmpty sx={{ borderRadius: 2 }}>
                <MenuItem value="">{t('videos.filters.all')}</MenuItem>
                <MenuItem value="true">{t('videos.status.active')}</MenuItem>
                <MenuItem value="false">{t('videos.status.inactive')}</MenuItem>
              </Select>
            </FormControl>
          </Box>
        </Box>
