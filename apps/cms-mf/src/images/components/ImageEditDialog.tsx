...
const ImageEditDialog = ({
  ...
  const availableTags = useMemo(
    () => getAppSettingTags('image_tags', i18n.language, formData.channel),
    ...
    >
      ...
      <DialogContent sx={{ pt: 3 }}>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5, pt: 2 }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'center', mb: 1 }}>
              <Box
                ...
                src={
                  ...
                  formData.fileUrl ||
                  getFullImageUrl(
                    formData.thumbnailFilename || formData.filename,
                  )
                }
                alt={formData.name}
                sx={{
          ...
          />
          <TextField
            label={t('images.form.originalUrl')}
          ...
          />
          <TextField
            label={t('images.form.filename')}
            value={formData.filename}
            onChange={(e) => onFormChange('filename', e.target.value)}
            fullWidth
          />
          <TextField
            label={t('images.form.thumbnailFilename')}
            value={formData.thumbnailFilename}
            onChange={(e) => onFormChange('thumbnailFilename', e.target.value)}
            fullWidth
          />
          ...
          <TextField
            label={t('images.form.displayOrder')}
          ...
          <FormControlLabel
            ...
            label={t('images.form.isActive')}
          />

          <TextField
            label={t('images.form.sourceName')}
          ...
          <TextField
            label={t('images.form.mimeType')}
            value={formData.mimeType}
            onChange={(e) => onFormChange('mimeType', e.target.value)}
            fullWidth
            error={!!getFieldError('mimeType')}
            helperText={getFieldError('mimeType')}
            disabled
          />
          <TextField
            label={t('images.form.sizeBytes')}
            value={formData.sizeBytes}
            onChange={(e) =>
              onFormChange('sizeBytes', parseInt(e.target.value) || 0)
            }
            type="number"
            fullWidth
            error={!!getFieldError('sizeBytes')}
            helperText={getFieldError('sizeBytes')}
            disabled
          />
          <TextField
            label={t('images.form.width')}
            value={formData.width}
            onChange={(e) =>
              onFormChange('width', parseInt(e.target.value) || 0)
            }
            type="number"
            fullWidth
          ...
          />
          <TextField
            label={t('images.form.height')}
            value={formData.height}
            onChange={(e) =>
              onFormChange('height', parseInt(e.target.value) || 0)
            }
            type="number"
            fullWidth
